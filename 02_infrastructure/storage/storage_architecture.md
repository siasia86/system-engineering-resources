...
서버 스토리지의 연결 방식(DAS, SAN, NAS, Object), SDS/HCI, 프로토콜, RAID, 설계 기준을 정리합니다.
...
| [5. Object Storage](#5-object-storage) / [6. SDS와 HCI](#6-sds와-hci) / [7. 비교](#7-비교) |
| [8. RAID](#8-raid) / [9. 프로토콜](#9-프로토콜) / [10. 설계 기준](#10-설계-기준)           |
| [11. 클라우드 스토리지](#11-클라우드-스토리지) / [12. 트러블슈팅](#12-트러블슈팅)          |
...
Object Storage는 데이터를 객체(Object) 단위로 저장하며, 각 객체는 고유 키(Key), 데이터(Value), 메타데이터로 구성됩니다. 파일시스템의 계층 구조 대신 플랫 네임스페이스를 사용하며, HTTP/REST API로 접근합니다.

### 특성

| 항목         | 내용                                              |
|--------------|---------------------------------------------------|
| 접근 레벨    | Object (HTTP/REST API)                            |
| 프로토콜     | S3 API, Swift API                                 |
| 네임스페이스 | 플랫 (Bucket + Key), 계층 구조 없음               |
| 확장성       | 무한 수평 확장 (PB~EB급)                          |
| 내구성       | 11 nines (99.999999999%) — 여러 AZ/노드 복제      |
| 성능         | 처리량 높음, 지연 높음 (ms~sec, 랜덤 I/O 부적합)  |
| 수정         | 불변(Immutable) — 덮어쓰기만 가능, 부분 수정 불가 |
| 비용         | GB당 가장 저렴 (HDD + Erasure Coding)             |
| 메타데이터   | 객체별 사용자 정의 메타데이터 태깅 가능           |

### Block vs File vs Object 비교

| 항목       | Block (DAS/SAN) | File (NAS)  | Object (S3)           |
|------------|-----------------|-------------|-----------------------|
| 접근 방식  | 섹터/블록 주소  | 경로/파일명 | Bucket + Key (HTTP)   |
| 수정       | 바이트 단위     | 바이트 단위 | 전체 객체 교체만      |
| 메타데이터 | 최소 (FS 의존)  | 파일 속성   | 풍부 (사용자 정의)    |
| 확장성     | TB급            | TB~PB급     | EB급                  |
| 지연       | us~ms           | ms          | ms~sec                |
| 용도       | DB, OS 디스크   | 파일 공유   | 백업, 로그, 정적 파일 |

### 주요 솔루션

| 솔루션     | 유형            | 비고                           |
|------------|-----------------|--------------------------------|
| AWS S3     | 퍼블릭 클라우드 | 사실상 Object Storage API 표준 |
| MinIO      | 오픈소스        | S3 호환, K8s 네이티브          |
| Ceph RGW   | 오픈소스        | S3/Swift 호환, 분산 스토리지   |
| Azure Blob | 퍼블릭 클라우드 | Hot/Cool/Archive 티어          |
| GCS        | 퍼블릭 클라우드 | Multi-regional 자동 복제       |

### 사용 환경

- 백업/아카이브 (S3 + Glacier, 수명주기 정책)
- 정적 웹 콘텐츠 (이미지, 동영상, JS/CSS)
- 로그 저장소 (CloudTrail, ALB 로그, 애플리케이션 로그)
- 데이터 레이크 (ML 학습 데이터, Parquet/ORC)
- 컨테이너 레지스트리 백엔드
...
SDS는 스토리지 관리를 하드웨어에서 분리하여 소프트웨어로 추상화하는 아키텍처입니다. 범용 서버의 로컬 디스크를 묶어 Block/File/Object 서비스를 제공합니다.

| 항목        | 내용                                             |
|-------------|--------------------------------------------------|
| 정의        | SW로 HW를 추상화하여 스토리지 서비스 제공        |
| 장점        | 벤더 종속 탈피, 범용 HW 사용, 유연한 확장        |
| 단점        | 운영 복잡도 증가, 성능 튜닝 필요                 |
| 제공 레벨   | Block + File + Object (통합 제공 가능)           |
| 주요 솔루션 | Ceph, VMware vSAN, NetApp ONTAP Select, Longhorn |
...
│                  Ceph Cluster                    │
├──────────┬──────────┬──────────┬─────────────────┤
│   RBD    │  CephFS  │   RGW    │ Access Layer    │
│  (Block) │  (File)  │ (Object) │                 │
├──────────┴──────────┴──────────┤                 │
│        RADOS (Core)            │ Distributed Obj │
├────────┬────────┬────────┬─────┤                 │
│  OSD   │  OSD   │  OSD   │ MON │ Daemons         │
│ (disk) │ (disk) │ (disk) │     │                 │
└────────┴────────┴────────┴─────┴─────────────────┘
```

- **RBD**: Block 디바이스 (K8s PV, VM 디스크)
- **CephFS**: POSIX 파일시스템 (NAS 대체)
- **RGW**: S3/Swift 호환 Object Gateway

### HCI (Hyper-Converged Infrastructure)

HCI는 Compute + Storage + Network를 단일 노드에 통합하여 SW로 관리하는 아키텍처입니다. 전통적인 3-tier(서버 + SAN + 스위치) 구조를 대체합니다.

| 항목        | 내용                                           |
|-------------|------------------------------------------------|
| 정의        | Compute + Storage + Network 통합 어플라이언스  |
| 장점        | 단순 관리, 빠른 배포, 노드 추가로 선형 확장    |
| 단점        | Compute/Storage 독립 확장 어려움               |
| 주요 솔루션 | Nutanix, VMware vSAN, Azure Stack HCI, Proxmox |
| 용도        | VDI, 지사/원격 사이트, 중소기업 인프라 통합    |

#### 3-Tier vs HCI 비교

| 항목      | 3-Tier (전통)             | HCI                           |
|-----------|---------------------------|-------------------------------|
| 구성      | 서버 + SAN + FC 스위치    | 통합 노드 × N                 |
| 스토리지  | 전용 SAN 장비             | 각 노드 로컬 디스크 (SW 풀링) |
| 확장      | 계층별 독립 확장          | 노드 단위 확장                |
| 비용      | 초기 고가, 전문 인력 필요 | 초기 저렴, 운영 단순          |
| 관리      | 별도 SAN 관리자 필요      | 단일 관리 콘솔                |
| 적합 환경 | 대규모 엔터프라이즈 DC    | 중소규모, 지사, VDI           |

### Unified Storage

Unified Storage는 하나의 장비에서 SAN(Block)과 NAS(File)을 동시에 제공하는 통합 스토리지입니다.

| 항목        | 내용                                         |
|-------------|----------------------------------------------|
| 정의        | Block + File 동시 제공 단일 장비             |
| 프로토콜    | FC/iSCSI (Block) + NFS/SMB (File)            |
| 장점        | 장비 통합, 관리 단순화, 비용 절감            |
| 주요 솔루션 | NetApp FAS/AFF, Dell PowerStore, HPE Alletra |
| 용도        | Block+File 모두 필요한 중대규모 환경         |
