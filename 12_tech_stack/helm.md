...
> ⚠️ **Helm v4 출시 (2025)**: v4는 v3와 대부분 호환되나 일부 CLI 플래그/SDK 변경이 있습니다. 기존 apiVersion v2 차트는 v4에서도 동작합니다. 주요 변경: WebAssembly 플러그인 시스템, Server-side Apply 기본 지원, 재현 가능한 차트 빌드.
...
⚠️ `--atomic` 플래그는 배포 실패 시 자동 롤백하지만, 롤백도 실패할 수 있습니다. 프로덕션에서는 `helm history`로 이력을 확인하고 수동 롤백을 병행합니다.

⚠️ Secret 값을 values.yaml에 평문으로 저장하지 않습니다. `helm-secrets` 플러그인 또는 외부 시크릿 관리자를 사용합니다.

⚠️ Chart 버전(`version`)과 앱 버전(`appVersion`)을 구분합니다. Chart 구조 변경 시 Chart 버전을 올립니다.
