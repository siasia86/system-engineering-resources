...
🟡 `prune: true`와 `selfHeal: true`를 프로덕션에서 활성화할 때는 Git 저장소 접근 제어를 먼저 강화합니다. Git이 단일 진실 공급원이므로 잘못된 커밋이 즉시 프로덕션에 반영됩니다.

🟡 ArgoCD UI/API는 외부에 노출하지 않습니다. VPN 또는 내부 네트워크에서만 접근하도록 설정합니다.
