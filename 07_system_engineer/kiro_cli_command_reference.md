...
| 단계              | 섹션                                                    |
|-------------------|---------------------------------------------------------|
| 기본              | 1.명령어 전체 목록 / 2.컨텍스트·도구 상세 / 3.단축키   |
| 핵심 개념         | 4.Context Window / 5.@ 참조 / 6.쉘 통합                |
| 일상 사용         | 7.도구 권한 / 8./plan 에이전트 / 9..kiro/ 디렉토리     |
| 커스터마이징      | 10.토큰·성능 / 11.Hooks / 12.Output Style / 13.실험적 기능 |
| 고급 활용         | 14.병렬 처리 / 15.CI/CD 비대화형 / 16.MCP 서버 연동  |
| 레퍼런스·부록     | 17.트러블슈팅 / 18.설정 전체 / 19.모델 / 20.대화 관리 |
| 부록              | 21.활용도 요약 / 22.참고 URL                              |
...
모델별 비용/속도 비교는 섹션 19(Model) 참고. `/model` 로 전환하면 비용과 속도를 최적화할 수 있습니다.
...
| MCP 서버 연결 실패                | 바이너리 미설치               | 섹션 15 MCP 트러블슈팅 참고                       |
