...
- [x] 검사 제외 항목에 사유·재검토 시점 기록 강제 (2026-08-27 완료)
...
| 항목                                        | 판정      | 근거                               |
|---------------------------------------------|-----------|------------------------------------|
| `infra_monorepo_and_boilerplate.md` (table) | 제거      | 제외 해제 후 0건 — 근거 소멸       |
| `privacy_law_guide.md` (table)              | 제거      | 제외 해제 후 0건 — 근거 소멸       |
| `vim_airline.md` (diagram-width, h1)        | 축소      | 실제 발생은 `emoji-disallow` 1건뿐 |
| `linux_setting.md`, `README_web.md`         | 사유 확정 | `/root/sj_del/` 소속 파일          |
| 나머지 5건                                  | 사유 확정 | 제외 해제 시 실제 이슈 발생 확인   |
...
- [ ] [Repository Governance 분리 계획](repository_governance_split_plan.md)을 기준으로 기존 분리 보류 결정을 재검토
      - 분리 실행 전 링크·CI·검증·배포·롤백 조건을 모두 확인
      - 결정 게이트 통과 전까지 현재 디렉토리를 정책 원본으로 유지
