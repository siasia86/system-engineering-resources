...
Python 데코레이터의 개념부터 고급 활용까지 다루는 가이드입니다.

## 목차

| 섹션 |
|------|
| [데코레이터란?](#1-데코레이터란) / [함수 데코레이터](#2-함수-데코레이터) / [클래스 데코레이터](#3-클래스-데코레이터) |
| [매개변수 있는 데코레이터](#4-매개변수-있는-데코레이터) / [functools 활용](#5-functools-활용) / [실전 예제](#6-실전-예제) |
| [실전 팁](#7-실전-팁) / [자주하는 실수](#8-자주하는-실수) / [요약](#9-요약) |
...
데코레이터 = 함수를 매개변수로 받아서, 새 함수를 리턴하는 함수

python
def 데코레이터(원래함수):       # 함수를 매개변수로 받음
    def 새함수(*args):
        print("전처리")
        원래함수(*args)         # 원래 함수 실행
        print("후처리")
...
Python에서 함수는 변수처럼 다룰 수 있어서 이게 가능합니다:
