...
| 섹션                                                                                                                                    |
|-----------------------------------------------------------------------------------------------------------------------------------------|
| [언어별 특징 요약](#언어별-특징-요약) / [1. C](#1-c) / [2. C++](#2-c)                                                                   |
| [3. C#](#3-c) / [4. Go](#4-go) / [5. Python](#5-python)                                                                                 |
| [6. Bash](#6-bash) / [7. 성능 비교](#7-성능-비교) / [8. 언어 선택 가이드](#8-언어-선택-가이드)                                          |
| [9. 실행 방식 상세 비교](#9-실행-방식-상세-비교) / [10. 메모리 관리 비교](#10-메모리-관리-비교) / [11. 동시성/병렬성](#11-동시성병렬성) |
| [12. 타입 시스템](#12-타입-시스템) / [13. 빌드 & 배포](#13-빌드--배포) / [14. 학습 난이도](#14-학습-난이도)                             |
| [15. 결론](#15-결론)                                                                                                                    |
...
# 장시간 실행 성능은 workload와 JIT warm-up에 따라 달라짐
```
...
| **Nuitka**      | workload 의존 | 높음        | 성능 + 배포        |
| **Cython**      | workload 의존 | 중간        | 성능 critical 부분 |
| **PyPy**        | workload 의존 | 낮음        | 장시간 실행        |
...
**단점:**
- 네이티브 언어보다 느릴 수 있음 (워크로드·구현에 따라 달라짐)
- GIL로 인한 멀티스레딩 제약
- 런타임 타입 에러
- 메모리 사용량 많음
- 배포 복잡 (의존성 관리)
...
언어별 실행 시간과 메모리 사용량은 compiler·runtime 버전, 최적화 옵션,
하드웨어, 작업량, 입력 데이터에 따라 달라집니다. 정량 비교가 필요하면
동일한 조건과 재현 가능한 측정 방법을 명시한 뒤 별도로 측정합니다.
