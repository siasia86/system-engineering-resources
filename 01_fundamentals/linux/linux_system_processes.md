...
systemd를 사용하는 환경에서 모든 사용자 프로세스의 부모 역할을 합니다. 부팅 시 PID 1로 실행되어 서비스 시작·종료·의존성을 관리합니다.
...
SAN·iSCSI 등 다중 경로 스토리지를 관리합니다. 경로 장애 시 다른 경로로 전환하여 고가용성을 제공합니다. AWS EBS 환경의 실행 여부는 배포판과 인스턴스 구성에 따라 별도로 확인합니다.
