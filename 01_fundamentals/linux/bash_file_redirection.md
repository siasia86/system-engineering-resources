...
<!-- reference: _reference/linux_kernel_official_notes.md, _reference/bash_shell_official_notes.md -->
...
```
  ...
  keyboard ───> │ 0 (stdin)    │                                             
                ...
                │ 2 (stderr) ──┼──┘                                          
                └──────────────┘                                             
                                    2>&1 = "redirect FD2 to where FD1 points"
```
...
`exec`로 변경한 FD는 명시적으로 복원하거나 쉘을 종료해야 원래 상태로 돌아옵니다.

```bash
# 방법 1: 미리 백업 후 복원
exec 3>&1 4>&2              # 원래 stdout/stderr 백업
exec >> /var/log/script.log 2>&1

echo "파일로 출력"

exec 1>&3 2>&4              # 터미널로 복원
exec 3>&- 4>&-              # 백업 FD 닫기

echo "터미널로 출력"

# 방법 2: /dev/tty 로 직접 복원 (백업 없이)
exec >> /var/log/script.log 2>&1

echo "파일로 출력"

exec > /dev/tty 2>&1        # 터미널로 복원 (/dev/tty = 현재 터미널)

echo "터미널로 출력"

# 방법 3: 스크립트 전체에 적용 (복원 불필요)
# 스크립트 종료 시 자동으로 FD 닫힘
exec >> /var/log/script.log 2>&1
# ... 이후 모든 출력이 파일로 ...
```
...
```
  ...
  │ command │              │  grep   │                         
  │         │              │         │                         
  │ FD1 ────┼───pipe──────>│ FD0     │  stdout only            
  │ FD2 ────┼──────────────┼─────────┼──> terminal  (not piped)
  └─────────┘              └─────────┘                         
```
...
```
  ...
  │         │              │         │                      
  │ FD1 ────┼──┐           │         │                      
  │ FD2 ────┼──┴──pipe────>│ FD0     │  stdout + stderr both
  └─────────┘  2>&1        └─────────┘                      
```
...
```
  ...
  keyboard ───> │ 0 (stdin)    │                                            
                ...
                │   Process    │                                            
                │              │     ┌─────────────┐                        
                │ 1 (stdout) ──┼────>│ tee -a log  ├──> LOG_FILE_01 (file)  
                │              │  │  │             ├──> terminal    (screen)
                │ 2 (stderr) ──┼──┘  └─────────────┘                        
                └──────────────┘                                            
                         2>&1 = stderr also to tee                          
```
