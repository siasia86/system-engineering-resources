...
CPU 명령어 집합 아키텍처(ISA)의 두 가지 설계 철학을 비교합니다.
...
| 심화 | [9. 비유로 이해하는 CISC vs RISC](#9-비유로-이해하는-cisc-vs-risc) / [10. 엔디안](#10-엔디안endianness) |
| 심화 | [11. 캐시 구조](#11-캐시-구조-l1l2l3) / [12. 명령어 실행 과정](#12-명령어-실행-과정)                    |
| 부록 | [13. CISC/RISC 역사 타임라인](#13-ciscrisc-역사-타임라인)                                               |
...
CISC(Complex Instruction Set Computer)는 하나의 명령어로 복잡한 연산을 수행하는 설계 방식입니다. 명령어 수가 많고, 명령어마다 길이와 실행 사이클이 다릅니다.
...
RISC(Reduced Instruction Set Computer)는 단순하고 균일한 명령어로 빠른 실행을 추구하는 설계 방식입니다. 명령어 수가 적고, 대부분 1사이클에 실행됩니다.
...
현대 x86 CPU는 외부적으로 CISC 명령어를 받지만, 내부적으로 RISC 마이크로옵(micro-op)으로 변환하여 실행합니다.
...
ARM도 버전이 올라가면서 명령어가 추가되고, 마이크로아키텍처가 복잡해지고 있습니다.
...
현대 CPU에서 CISC/RISC의 경계는 흐려졌습니다. 핵심 차이는 ISA(명령어 집합) 수준의 설계 철학이며, 실제 실행 엔진은 양쪽 모두 유사한 기법을 사용합니다.
...
ARM 서버로 전환 시 소프트웨어 호환성을 먼저 확인합니다.
...
RISC-V는 오픈소스 ISA로, 라이선스 비용이 없어 IoT/임베디드에서 채택이 증가하고 있습니다. 서버 시장 진입은 아직 초기 단계이나 장기적으로 주목할 아키텍처입니다.
...
## 9. 비유로 이해하는 CISC vs RISC

🟢 비전공자/입문자도 직관적으로 이해 가능한 비유.

### 맥가이버 vs 전문가 팀

| 비유      | CISC (맥가이버)                   | RISC (전문가 팀)                |
|-----------|-----------------------------------|---------------------------------|
| 도구      | 만능 스위스 나이프 (하나로 다 함) | 전문 도구 세트 (각각 한 가지만) |
| 작업 방식 | 혼자서 복잡한 작업 수행           | 단순 작업을 빠르게 분업         |
| 속도      | 한 작업은 오래 걸림               | 각 작업은 빠름                  |
| 학습 비용 | 도구 사용법이 복잡                | 도구 사용법이 단순              |

### 식당 비유

```
CISC = Full-service restaurant
  Customer: "I want steak with salad and wine"
  Waiter:   handles everything in one trip
  --> 1 order, but takes long to prepare

RISC = Fast-food assembly line
  Station 1: grill patty
  Station 2: add lettuce
  Station 3: wrap burger
  Station 4: pour drink
  --> 4 steps, but each is fast (pipeline)
```

- CISC: 하나의 명령어가 여러 작업을 한 번에 처리 (풀서비스 레스토랑)
- RISC: 단순한 명령어를 파이프라인으로 빠르게 처리 (패스트푸드 조립 라인)

### 핵심 요약

| 관점        | CISC                    | RISC                  |
|-------------|-------------------------|-----------------------|
| 철학        | "하나로 다 하자"        | "단순하게, 빠르게"    |
| 복잡도      | 명령어가 복잡           | 하드웨어가 단순       |
| 최적화 주체 | 하드웨어 (마이크로코드) | 소프트웨어 (컴파일러) |
...
🟡 CPU 아키텍처와 밀접하며, 네트워크/파일 처리 시 실무 이슈가 됩니다.

### 개념

멀티바이트 데이터를 메모리에 저장하는 바이트 순서(byte order).

| 방식               | 저장 순서                    | 대표 아키텍처                |
|--------------------|------------------------------|------------------------------|
| Big-Endian (BE)    | 상위 바이트 먼저 (MSB first) | SPARC, POWER (네트워크 표준) |
| Little-Endian (LE) | 하위 바이트 먼저 (LSB first) | x86, ARM (기본), RISC-V      |
| Bi-Endian          | 설정으로 전환 가능           | ARM, MIPS, POWER (일부)      |

### 메모리 배치 예시

`0x12345678` 을 주소 `0x00` 부터 저장할 때:

```
Address:   0x00   0x01   0x02   0x03

Big-Endian:
           [0x12] [0x34] [0x56] [0x78]   MSB first
...
| 상황               | 문제                            | 해결                        |
|--------------------|---------------------------------|-----------------------------|
| 네트워크 통신      | 네트워크는 BE, 호스트는 LE      | `htonl()` / `ntohl()` 변환  |
| 바이너리 파일 교환 | 다른 엔디안 시스템 간 파일 공유 | 헤더에 엔디안 명시 (ELF 등) |
| 직렬화/역직렬화    | 바이트 순서 불일치              | 프로토콜에 엔디안 규약 정의 |
...
| 캐시 | 용량        | 지연 시간 | 공유 범위      | 특징                   |
|------|-------------|-----------|----------------|------------------------|
| L1-I | 32~64 KB    | ~1 ns     | 코어 전용      | 명령어 캐시            |
| L1-D | 32~64 KB    | ~1 ns     | 코어 전용      | 데이터 캐시            |
| L2   | 256 KB~1 MB | 3~5 ns    | 코어 전용      | 통합 캐시              |
| L3   | 8~64 MB     | 10~20 ns  | 전체 코어 공유 | LLC (Last Level Cache) |
| RAM  | GB 단위     | 50~100 ns | 전체 시스템    | 메인 메모리            |

### CISC vs RISC 캐시 차이

| 항목      | CISC (x86)                                           | RISC (ARM)                       |
|-----------|------------------------------------------------------|----------------------------------|
| L1 캐시   | 가변 길이 명령어 → 디코드 캐시 추가 (micro-op cache) | 고정 길이 → 단순 I-cache         |
| 캐시 효율 | 코드 밀도 높아 I-cache 적중률 양호                   | 코드 크기 커서 I-cache 미스 가능 |
| 전력      | 디코드 캐시로 전력 소모 증가                         | 단순 구조로 전력 효율 우수       |
...
| 단계           | 영문          | 동작                                                |
|----------------|---------------|-----------------------------------------------------|
| 1. 인출        | Fetch         | PC(Program Counter)가 가리키는 주소에서 명령어 읽기 |
| 2. 해독        | Decode        | 명령어를 해석하여 연산 종류, 피연산자 결정          |
| 3. 실행        | Execute       | ALU에서 연산 수행                                   |
| 4. 메모리 접근 | Memory Access | 필요 시 메모리 읽기/쓰기                            |
| 5. 쓰기        | Write Back    | 결과를 레지스터에 저장                              |

### CISC vs RISC 실행 비교

```
Example: A = B + C (B, C are in memory)

CISC (1 instruction, multiple cycles):
  Cycle 1:  Fetch  [ADD mem_B, mem_C, mem_A]
  Cycle 2:  Decode (complex, microcode lookup)
  Cycle 3:  Load B from memory
  Cycle 4:  Load C from memory
  Cycle 5:  Execute ADD
  Cycle 6:  Store result to A
  --> 1 instruction, 6 cycles

RISC (4 instructions, pipelined):
  Cycle:    1     2     3     4     5     6     7     8
  LDR R1:  [IF]  [ID]  [EX]  [MA]  [WB]
  LDR R2:        [IF]  [ID]  [EX]  [MA]  [WB]
  ADD R3:              [IF]  [ID]  [EX]  [MA]  [WB]
  STR R3:                    [IF]  [ID]  [EX]  [MA]  [WB]
  --> 4 instructions, completes in 8 cycles
...
```
1960s         1980s         1995          2020s
  |             |             |             |
  v             v             v             v
CISC 탄생 --> RISC 등장 --> CISC+RISC --> ARM/RISC-V
(IBM 360)   (Berkeley,    융합         서버 시장
             Stanford)   (Pentium Pro)  본격 진입
```
