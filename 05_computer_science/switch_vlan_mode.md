...
스위치 포트의 VLAN 모드는 프레임이 포트를 통과할 때 VLAN 태그를 어떻게 처리하는지를 결정한다.
...
하나의 VLAN에만 소속되는 포트 모드. 주로 PC, 프린터 등 엔드 디바이스를 연결할 때 사용한다.
...
여러 VLAN의 트래픽을 하나의 링크로 전달하는 포트 모드. 스위치 간 연결, 스위치-라우터 간 연결에 사용한다.
...
⚠️ 양쪽 스위치의 Native VLAN이 불일치하면 트래픽이 잘못된 VLAN으로 전달될 수 있다.
...
IP 전화기(IP Phone)를 연결할 때 데이터 VLAN과 음성 VLAN을 분리하는 기능. Access 포트에서 추가로 설정한다.
...
하나의 VLAN 내에서 포트 간 통신을 제한하는 기능. 같은 서브넷에 있지만 호스트 간 직접 통신을 차단할 때 사용한다.
...
공격자가 DTP 패킷을 전송하여 스위치와 Trunk를 형성, 모든 VLAN 트래픽을 수신한다.
...
Native VLAN이 기본값(VLAN 1)일 때, 이중 802.1Q 태그를 삽입하여 다른 VLAN으로 프레임을 전달한다.
...
ISL은 현재 거의 사용하지 않으며, 신규 환경에서는 802.1Q만 사용한다. 구형 장비 마이그레이션 시에만 참고.
...
VLAN 1을 Native VLAN으로 사용하면 Double Tagging 공격에 취약하다.
...
미사용 포트를 열어두면 물리적 접근만으로 네트워크 침입이 가능하다.
...
한쪽만 Trunk이고 반대쪽이 Access이면 통신 장애가 발생한다. Native VLAN 불일치도 CDP/로그에서 경고가 나오므로 확인할 것.
...
VTP Server/Client 모드에서 실수로 VLAN DB가 전파되어 전체 네트워크 VLAN이 삭제되는 사고를 방지한다. 대규모 환경에서는 VTP를 사용하지 않거나 Transparent 모드를 권장.
...
VLAN/포트 변경 전 현재 설정을 저장해두면 롤백이 가능하다.
