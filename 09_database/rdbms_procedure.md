...
Stored Procedure는 **데이터베이스 서버에 저장된 SQL 코드 블록**입니다.
...
🟡 커서는 행 단위 처리로 성능이 낮습니다. 가능하면 집합 기반(SET-based) SQL로 대체합니다.
...
복잡한 중간 결과를 임시 테이블에 저장하면 반복 계산을 줄일 수 있습니다.
...
루프 내 매 행마다 COMMIT하면 I/O 오버헤드가 큽니다.
