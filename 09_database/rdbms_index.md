...
Index는 테이블의 특정 컬럼에 대한 **검색 속도를 높이기 위한 자료구조**다.
책의 색인처럼 원하는 데이터의 위치를 빠르게 찾아준다.
...
인덱스가 많을수록 INSERT/UPDATE/DELETE 성능이 저하된다.
...
INSERT/UPDATE/DELETE가 반복되면 인덱스 페이지가 단편화되어 성능이 저하된다.
...
옵티마이저는 통계 정보를 기반으로 실행 계획을 수립한다.
대량 데이터 변경 후 통계를 갱신해야 정확한 실행 계획이 생성된다.
