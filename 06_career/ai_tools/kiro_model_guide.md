...
| 섹션                                                                                                                              |
|-----------------------------------------------------------------------------------------------------------------------------------|
| [모델 목록](#1-모델-목록) / [용도별 추천](#2-용도별-추천) / [인프라 엔지니어 시나리오별 추천](#3-인프라-엔지니어-시나리오별-추천) |
| [모델 변경 방법](#4-모델-변경-방법) / [에이전트별 모델 고정](#5-에이전트별-모델-고정) / [크레딧 확인](#6-크레딧-확인)             |
| [선택 우선순위](#7-선택-우선순위)                                                                                                 |
...
| `claude-opus-4.6`   | 2.20x       | 1M       | 복잡한 추론에 적합, 최신 Opus |
...
| 복잡한 아키텍처 설계         | `claude-opus-4.6`   | 복잡한 추론에 적합, 긴 컨텍스트 필요 시 |
...
```
┌───────────────────────────┬───────────────────────────┬───────────────────────────┐
│ High Cost / Quality       │ Medium                    │ Low Cost / Fast           │
│ claude-opus-4.6 (2.20x)   │ claude-sonnet-4.6 (1.30x) │ claude-haiku-4.5 (0.40x)  │
├───────────────────────────┼───────────────────────────┼───────────────────────────┤
│ - Architecture Design     │ - Code Write/Review       │ - Command Lookup          │
│ - Root Cause Analysis     │ - Documentation           │ - Simple Q&A              │
│ - Migration Planning      │ - Ansible/Terraform       │ - Repetitive Automation   │
│ - Security Audit          │ - Log Analysis            │ - CI/CD Pipeline          │
└───────────────────────────┴───────────────────────────┴───────────────────────────┘
```

- High Cost / Quality: 아키텍처 설계, 장애 원인 분석, 마이그레이션 계획, 보안 취약점 분석
- Medium: 코드 작성/리뷰, 문서 작성, Ansible/Terraform, 로그 분석
- Low Cost / Fast: 명령어 확인, 간단한 질문, 반복 자동화, CI/CD 파이프라인
