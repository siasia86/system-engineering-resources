...
def check_code_lang(content, strict=False):
    """언어 태그 없는 코드블록 검사. 트리/다이어그램(├──, └──, │, ┌) 포함 블록은 허용."""
    issues = []
    for m in re.finditer(r'```([^\n]*)\n(.*?)```', content, re.DOTALL):
        ...
        if lang:
            continue
        # 트리/다이어그램/순수 텍스트 출력 — 허용
        if any(c in body for c in ['├──', '└──', '│', '┌', '┐', '└', '┘', '─']):
            continue
        # 첫 줄이 URL이면 허용 (repo URL 패턴 등)
        first_line = body.strip().splitlines()[0] if body.strip() else ''
        if first_line.startswith('http'):
            continue
        issues.append(f"언어 태그 없는 코드블록: '{body[:40].strip()}'")
    return issues
...
def check_diagram(content, strict=False):
    """박스 다이어그램(┌┐└┘로 둘러싸인 것) 내부 행 display width 일치 여부.
    트리 구조(├──, └──)와 흐름도(v, │)는 검사 제외."""
    issues = []
    for m in re.finditer(r'```[^\n]*\n(.*?)```', content, re.DOTALL):
        body = m.group(1)
        lines = body.splitlines()
        # 박스 다이어그램: ┌ 또는 ┐ 로 시작하는 행이 있어야 함
        if not any(l.strip().startswith('┌') or l.strip().startswith('┐') for l in lines):
            continue
        content_lines = [l for l in lines if l.strip()]
        if not content_lines:
            continue
        widths = [dw(l) for l in content_lines]
        max_w = max(widths)
        for l, w in zip(content_lines, widths):
            if w != max_w:
                issues.append(
                    f"다이어그램 행 폭 불일치: dw={w} (최대={max_w}) | '{l[:50]}'"
                )
    return issues
...
def main():
    ...
    print(f"\n{'─'*60}")
    print(f"검사 파일: {len(files)}개 | 이슈: {total_issues}건")
