...
| 섹션 |
|------|
| [문서 목록](#문서-목록) / [상황별 도구 선택 가이드](#상황별-도구-선택-가이드) / [체크리스트](#체크리스트) |
...
### 디버깅 도구
4. **[gdb.md](gdb.md)** - 소스 레벨 디버거
   - 용도: 크래시 분석, 로직 버그
   - 난이도: ⭐⭐⭐
   - 오버헤드: 높음 (중단점 사용)

5. **[valgrind.md](valgrind.md)** - 메모리 디버깅
   - 용도: 메모리 누수, 버퍼 오버플로우
   - 난이도: ⭐⭐
   - 오버헤드: 매우 높음 (20-50배)

### 시스템 모니터링 도구
6. **[lsof.md](lsof.md)** - 열린 파일 추적
   - 용도: 파일/포트 사용 확인
   - 난이도: ⭐
   - 오버헤드: 매우 낮음

7. **[iotop.md](iotop.md)** - I/O 모니터링
   - 용도: 디스크 I/O 병목
   - 난이도: ⭐
   - 오버헤드: 매우 낮음

8. **[tcpdump.md](tcpdump.md)** - 네트워크 패킷 캡처
   - 용도: 네트워크 문제, 프로토콜 분석
   - 난이도: ⭐⭐⭐
   - 오버헤드: 중간
