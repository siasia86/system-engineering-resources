...
# Requires: bash 4.0+, grep, find, du (GNU coreutils)
# Created by sjyun on 2026-02-02. Version 26.5.4 Modified by sjyun on 2026-05-04.
#
...
# ---------------------------------------------------------------------------
# find_files PATTERN — 검사 대상 파일에서 패턴 검색 (파일명 반환)
# ---------------------------------------------------------------------------
find_files() {
    local pattern="$1"
    find "$SCAN_DIR" -type d -name ".git" -prune -o \
        ! \( -name "git-security-check.sh" \
            ...
            -o -name "*.zip" -o -name "*.jar" -o -name "*.pdf" \
            -o -name "*.out" -o -name "*.bin" -o -name "*.bak" \
            -o -name "*.trn" -o -name "*.swp" -o -name "*.swo" \
            -o -name "*.log" -o -name "*.map.json" \
            -o -name "*_rsyncd.conf" -o -name "*rsyslog.conf" \
        \) -type f -print0 2>/dev/null | \
        xargs -0 grep -IlE "$pattern" 2>/dev/null || true
}

# ---------------------------------------------------------------------------
# find_files_content PATTERN — 파일명:내용 형식으로 반환
# ---------------------------------------------------------------------------
find_files_content() {
    local pattern="$1"
    find "$SCAN_DIR" -type d -name ".git" -prune -o \
        ! \( -name "git-security-check.sh" \
            ...
            -o -name "*.png" -o -name "*.zip" -o -name "*.jar" \
            -o -name "*.pdf" -o -name "*.bak" -o -name "*.trn" -o -name "*.swp" \
        \) -type f -print0 2>/dev/null | \
...
# ---------------------------------------------------------------------------
# check_sensitive_ips — [1/5] 민감 IP 검사
# ---------------------------------------------------------------------------
check_sensitive_ips() {
...
# ---------------------------------------------------------------------------
# check_passwords — [2/5] 패스워드/키 패턴 검사
# ---------------------------------------------------------------------------
check_passwords() {
    echo ""
    ...
    for pattern in "${SENSITIVE_PATTERNS[@]}"; do
        local raw results filtered
        raw=$(find_files_content "$pattern")

        results=$(echo "$raw" | \
            grep -v "example" | grep -v "placeholder" | \
            grep -v "SecurePassword123" | grep -v "SecureToken123" | \
            grep -v "SecureKey123" | grep -v "Secureuser123" | \
            grep -v "your_password" | grep -v "='secret'" | grep -v "=local" | grep -v "SecretString"  | grep -v "response['SecretString']" | grep -v "ansible_ssh_private_key_file" | \
            grep -v "비밀번호" | grep -v '="$' | \
            grep -vE '=["'"'"'](\$|\$\{)' | \
            grep -vE '=\$?\(' || true)
        ...
        if [ -n "$filtered" ]; then
            ...
            echo "$filtered" | head -$MAX_LINES | while IFS=: read -r file content; do
                local highlighted="$content"
                for excl in "SecurePassword123" "SecureToken123" "SecureKey123" "Secureuser123" \
                            "your_password" "example" "placeholder" "비밀번호" "='secret'" \
                            "=local" "ansible_ssh_private_key_file" "SecretString" "response['SecretString']"; do
...
# ---------------------------------------------------------------------------
# check_aws_account_ids — [3/5] AWS 계정 ID 검사
# ---------------------------------------------------------------------------
check_aws_account_ids() {
    ...
    results=$(find "$SCAN_DIR" -type d \( -name ".git" -o -path "*/02_fio*" \) -prune -o \
        ! \( -name "git-security-check.sh" -o -name "aws-security-check.sh" \
            -o -name "gitlab.rb" -o -name "GITHUB_UPLOAD_GUIDE.md" \
            -o -name "*.tar" -o -name "*.tar.gz" -o -name "*.png" \
            -o -name "*.zip" -o -name "*.bak" -o -name "*.trn" \
            -o -name "*.swp" -o -name "*.map.json" \
        \) -type f -print0 2>/dev/null | \
        xargs -0 grep -IoE "[0-9]{12}" 2>/dev/null | grep -v '123456789012' | grep -vE 'https?://[^ ]*[0-9]{12}' | \
        sort -t: -k2 -u || true)
...
# ---------------------------------------------------------------------------
# check_large_files — [4/5] 대용량 파일 검사
# ---------------------------------------------------------------------------
check_large_files() {
    echo ""
...
# ---------------------------------------------------------------------------
# check_sensitive_filenames — [5/5] 민감 파일명 검사
# ---------------------------------------------------------------------------
check_sensitive_filenames() {
    ...
    local SENSITIVE_FILES=(
        "*.pem" "*.key" "*.p12" "*.pfx" "*.jks"
        "id_rsa" "id_dsa" "*.env" ".env.*"
        "credentials" "secret*"
    )

    for pattern in "${SENSITIVE_FILES[@]}"; do
        local results md_results
        results=$(find "$SCAN_DIR" -type d \( -name ".git" -o -path "*/06_docker/02_n8n_docker*" \) -prune -o \
            -type f -name "$pattern" ! -name "*.md" -print 2>/dev/null || true)
        md_results=$(find "$SCAN_DIR" -type d \( -name ".git" -o -path "*/06_docker/02_n8n_docker*" \) -prune -o \
            -type f -name "$pattern" -name "*.md" -print 2>/dev/null || true)
        if [ -n "$results" ]; then
        ...
        if [ -n "$md_results" ]; then
            echo -e "${YELLOW}  ⚠ Found sensitive-named .md files: $pattern${NC}"
            echo "$md_results" | while read -r file; do
