...
| 섹션 |
|------|
| [구성도](#구성도) / [1단계: A계정 - S3 Gateway Endpoint 생성](#1단계-a계정---s3-gateway-endpoint-생성) / [2단계: B계정 - S3 버킷 정책 (크로스 계정 허용)](#2단계-b계정---s3-버킷-정책-크로스-계정-허용) |
| [3단계: A계정 - IAM 정책 (EC2에서 B계정 S3 접근)](#3단계-a계정---iam-정책-ec2에서-b계정-s3-접근) / [4단계: A계정 - VPC Endpoint 정책 (선택)](#4단계-a계정---vpc-endpoint-정책-선택) / [5단계: 테스트](#5단계-테스트) |
| [확인 체크리스트](#확인-체크리스트) / [AWS 콘솔(웹)에서 설정하는 방법](#aws-콘솔웹에서-설정하는-방법) / [주의사항](#주의사항) |
...
> 🟡 `aws:sourceVpce` 조건은 선택사항입니다. VPC Endpoint를 통한 접근만 허용하려면 추가하고, 제한 없이 A계정 전체를 허용하려면 `Condition` 블록을 제거합니다.
