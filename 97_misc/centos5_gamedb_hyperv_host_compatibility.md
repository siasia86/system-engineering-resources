...
```
VM 메모리 주소 공간 (4GB, 32-bit PAE)

0x00000000 ┌──────────────────────────────┐
           │                              │
           │   사용 가능한 RAM            │
           │                              │
0xC0000000 ├──────────────────────────────┤  ← MMIO gap (기본 1GB)
           │   PCI config, VGA, APIC 등   │
0xFFFFFFFF └──────────────────────────────┘
```
...
```
I/O 경합 (다른 VM noisy neighbor)
         │
         v
SCSI timeout 60s (디스크 응답 없음)
         │
         v
EXT4 journal abort → filesystem read-only
         │
         v
InnoDB doublewrite 중단
         │
         v
┌─────────────────────────────────────────┐
│  최선: crash recovery 정상 완료         │
│  최악: 페이지 손상 → 테이블 복구 필요   │
└─────────────────────────────────────────┘
```
...
- [97_misc/hyperv_version_centos5_compatibility.md](hyperv_version_centos5_compatibility.md)
