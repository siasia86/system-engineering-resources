...
`md-heading-check.py` 에 `toc` 검사를 추가했습니다. 본문의 번호 있는 H2가 목차에 없으면 검출합니다. `anchor` 검사가 목차에서 본문 방향을 보는 것과 반대 방향입니다.

구현 중 기존 결함 2건을 발견했습니다.

- `NUMBERED_H2_PATTERN` 이 이중 이스케이프로 작성되어 리터럴 백슬래시를 찾고 있었습니다. `number` 검사가 도입 이후 아무것도 검사하지 않았습니다.
- `ANCHOR_LINK_PATTERN` 이 링크 텍스트의 중첩 대괄호를 처리하지 못했습니다. 링크 텍스트 안에 대괄호가 한 겹 들어간 형태를 인식하지 못해 `anchor` 검사에서 누락됐습니다.

두 패턴을 고친 뒤 목차 누락 5건을 검출해 수정했습니다.
