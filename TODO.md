...
- [x] `04_test_design/concurrency_testing.md` — 동시 실행·경쟁 조건·교착 상태 검증
- [x] `04_test_design/fault_injection_testing.md` — 장애 주입 기반 테스트 설계
- [x] `04_test_design/exploratory_testing.md` — 탐색적 테스트의 목적과 실행 전략
- [x] `04_test_design/property_based_testing.md` — 속성·불변 조건 기반 테스트
- [x] `02_white_box/mutation_testing.md` — 코드 변형 기반 테스트 품질 평가
...
- [x] `gameplay_scenario_testing.md` — 게임 플레이 흐름과 사용자 시나리오 검증
- [x] `save_load_testing.md` — 저장·불러오기·중단 후 복구 검증
- [x] `matchmaking_testing.md` — 매칭·방·세션 규칙 검증
- [x] `game_economy_testing.md` — 재화·보상·인벤토리 정합성 검증
- [x] `game_performance_testing.md` — 게임 도메인의 성능 지표와 테스트 관점
...
- [x] 게임 사용자 여정별 부하 시나리오 — 로그인·로비·매칭·게임·보상 흐름
- [x] 실시간 연결 및 재연결 시나리오 — 장기 연결·재접속 폭주·세션 복구
- [x] 게임 데이터 정합성 시나리오 — 재화·보상·랭킹·결제 중복 처리
- [x] Node.js·C#·Unity 런타임별 관측 지표 — Event Loop·GC·ThreadPool·Frame/Tick
...
- [x] 범용 고급 테스트 문서 → `_reference/advanced_testing_official_notes.md`
  - 대상: `fault_injection`은 `_reference/chaos_finops_official_notes.md`도 추가 참조하며, `property_based`, `mutation_testing`은 고급 테스트 reference를 참조
...
- [x] 공통 reference를 연결한 본문 `.md`는 문서 대상 fact-check 회차 규칙(3회)을 별도로 적용
- [x] 새로 생성한 공통 reference 작성·INDEX 등록·`@fact-check` 2회 완료
- [x] 본문 작성 후 Markdown style·heading·link 검사를 실행
...
| 8-2  | [x]  | 사용자 여정·실시간 연결·재연결 시나리오                     | 서비스 부하 시나리오 정리    |
...
- [x] 메인 작업 Batch 1~7 및 보조 Batch 8-1·8-2 완료

현재 메인 문서 작업은 완료되었으며, Batch 8-3은 실제 서비스 승인값이 필요한 별도 운영 입력으로 TODO에서 관리하지 않습니다.
