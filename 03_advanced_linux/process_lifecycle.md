...
```
Child (PID:1001, bash 복사본)
    │ exec("nginx")
    v
nginx (PID:1001)  ← 같은 PID, 다른 프로그램
```
...
```
┌──────────┐         ┌──────────┐
│  Parent  │         │  Child   │ ← exit() 완료
│ (실행 중)│         │ (Zombie) │   wait() 미호출
└──────────┘         └──────────┘
                     PID 항목만 남음
```
...
httpd master (PID:1000)
    │ fork() × N
    ├── httpd worker (PID:1001)  ← prefork: 요청 1개 처리
